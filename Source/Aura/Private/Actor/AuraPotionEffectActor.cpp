...
AAuraPotionEffectActor::AAuraPotionEffectActor()
{
	...
	static ConstructorHelpers::FObjectFinder<UStaticMesh> MeshAsset(
		TEXT("/Script/Engine.StaticMesh'/Game/Assets/Pickups/Potions/SM_PotionBottle.SM_PotionBottle'"));
                                       ...
                                       const FHitResult& SweepResult)
{
	if (IAbilitySystemInterface* ASCInterfae = Cast<IAbilitySystemInterface>(OtherActor))
	{
		const UAuraAttributeSet* AuraAttributeSet = Cast<UAuraAttributeSet>(
			ASCInterfae->GetAbilitySystemComponent()->GetAttributeSet(UAuraAttributeSet::StaticClass()));
		UAuraAttributeSet* MutAttributeSet = const_cast<UAuraAttributeSet*>(AuraAttributeSet);

		MutAttributeSet->SetHealth(MutAttributeSet->GetHealth() + 25.0f);
		MutAttributeSet->SetMana(MutAttributeSet->GetMana() - 25.0f);
